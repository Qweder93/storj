...
func (credits *credit) ListCreditsSpendingsPaged(ctx context.Context, status stripecoinpayments.CreditsSpendingStatus, offset int64, limit int, before time.Time) (_ stripecoinpayments.CreditsSpendingsPage, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	dbxSpendings, err := credits.db.Limited_CreditsSpending_By_CreatedAt_LessOrEqual_And_Status_OrderBy_Desc_CreatedAt(
		...
		dbx.CreditsSpending_Status(int(status)),
		limit+1,
		offset,
	)
	if err != nil {
